...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
	{
		//checks if there is a filename in the command line
	...
	{
		//for loop iterates over the input samples 
		if (it.key() == "metadata")
		{
			//doesn't check metadata because it is not a sample
		...
		{
			//for loop iterates through the sample to check if it is sorted
			if (sample[i] > sample[i + 1])
			{
				//compares consecutive sample array indexes 
		...
		if (!is_sorted)
		{
			//if checks if the json is not sorted 
